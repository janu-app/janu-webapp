  ...
  <home-layout>
    <section class="section">
      <div class="container">
        ...
        <h5 class="subtitle is-5">Aquí podrás ver las clases</h5>
              ...
              <div class="control">
                <div class="select">
                  <select v-model="form.grado" @change="onChangeGrado">
                    <option v-for="grado in grados" :key="grado.classroomId" :value="grado.classroomId">{{grado.name}}</option>
                  </select>
                </div>
              </div>
            </div>
        ...
        <table class="table">
          <tbody>
            <tr v-for="student in students" :key="student.lastname+student.lastname2+student.name">
...
export default {
  ...
  computed: {
    ...
    grados() {
      const grados = this.response.results.filter(r => r.turno == this.form.turno)
      if (grados.length) {
